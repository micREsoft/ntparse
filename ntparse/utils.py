...
def get_default_ntdll_path() -> str:
    # get the default path to ntdll.dll based on the current system
    # returns: path to the default ntdll.dll
    system_root = os.environ.get('SystemRoot', 'C:\\Windows')
    return os.path.join(system_root, 'System32', 'ntdll.dll')

def find_ntdll_files() -> List[str]:
    # find all ntdll.dll files on the system
    # returns: list of paths to ntdll.dll files
    ntdll_paths = []
    # common locations
    common_paths = [
...
def get_pe_info(file_path: str) -> Dict:
    ...
    # args: file_path: path to the PE file
...
def filter_nt_functions(functions: List[str]) -> List[str]:
    ...
    # args: functions: list of function names
...
def format_syscall_number(number: int, format_type: str = "hex") -> str:
    ...
    # args: number: the syscall number, format_type: format type ("hex", "decimal", "both")
...
def create_syscall_table(syscalls: Dict[str, int], include_zw: bool = True) -> Dict[str, Dict[str, int]]:
    ...
    # args: syscalls: dictionary of syscall names to numbers, include_zw: whether to include Zw functions
