...
def to_json(syscalls: Dict[str, int], output_file: TextIO = None) -> str:
    ...
    # args: syscalls: dictionary of syscall names to numbers, output_file: optional file object to write to
    # returns: json string representation
    # convert decimal numbers to hex offsets
    offsets = get_offsets(syscalls)
    # sort syscalls by number for consistent output
...
def to_csv(syscalls: Dict[str, int], output_file: TextIO = None) -> str:
    ...
    for func_name, hex_offset in sorted_offsets:
        # extract decimal number from hex offset for display
...
def to_asm(syscalls: Dict[str, int], output_file: TextIO = None) -> str:
    ...
    # args: syscalls: dictionary of syscall names to numbers, output_file: optional file object to write to
    # returns: assembly string representation
    # convert decimal numbers to hex offsets for assembly
...
def to_python_dict(syscalls: Dict[str, int], output_file: TextIO = None) -> str:
    ...
    # args: syscalls: dictionary of syscall names to numbers, output_file: optional file object to write to
    # returns: python dictionary string representation
    # convert decimal numbers to hex offsets
    offsets = get_offsets(syscalls)
    # sort syscalls by number
    sorted_offsets = sorted(offsets.items(), key=lambda x: int(x[1], 16))
